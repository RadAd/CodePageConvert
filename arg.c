...
static void AddArgArgDescription(const TCHAR* arg, const TCHAR* value, const TCHAR* desc)
{
    ArgArgDescription* argargdescription = &g_argargdescription[g_argargdescriptioncount++];
    argargdescription->arg = arg;
    argargdescription->value = value;
    argargdescription->desc = desc;
}

static ArgArgNumDescription* AddArgNumDescription(const TCHAR* descvalue, const TCHAR* desc)
{
...
#define msg(...) _ftprintf(stdout, __VA_ARGS__)
    ...
    if (g_argargdescriptioncount > 0)
    {
        ...
        for (int i = 0; i < g_argargdescriptioncount; ++i)
        {
        ...
        for (int i = 0; i < g_argargnumdescriptioncount; ++i)
        {
            const ArgArgNumDescription* argargnumdescription = &g_argargnumdescription[i];
...
{
    AddArgArgDescription(argf, desc, NULL);
    for (int argi = 1; argi < g_argc; ++argi)
    {
        const TCHAR* arg = g_argv[argi];
...
{
    AddArgArgDescription(argf, descvalue, desc);
...
{
    const TCHAR* v = argvalue(argf, NULL, descvalue, desc);
    if (v == NULL)
        return def;
    else
        return _tstoi(v);
...
{
    ArgArgNumDescription* argargnumdescription = AddArgNumDescription(descvalue, desc);
